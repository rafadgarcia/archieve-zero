---
id: PES-OSHINO
titulo: Oshino
categoria: Pessoas
status: Atualização registrada
fonte: 02_DOSSIES_INVESTIGATIVOS/Fichamento_Aliados_e_Ameacas.md
secao: 1. Aliados e Membros da Ordem — Oshino (Protagonista)
tags: ocultista, lâmina maldita, câmera, projeto k
---

# PROTOCOLO OSHINO

> Documento pessoal criado para preservar decisões, valores e prioridades de Oshino após a perda dos sentimentos.

\---

# 1\. Condição atual

Oshino perdeu seus sentimentos como preço por uma resposta do Outro Lado.

Isso inclui, até onde se sabe:

* empatia;
* medo;
* culpa;
* afeto;
* amor;
* ódio;
* tristeza;
* alegria;
* saudade;
* apego emocional.

Ainda não está confirmado se ele perdeu:

* valores;
* memórias;
* senso de identidade;
* capacidade de escolher;
* capacidade de reconhecer relações;
* princípios morais aprendidos.

\---

# 2\. Regra principal

> \*\*Não confundir ausência de sentimento com ausência de decisão.\*\*

O Oshino atual pode não sentir que algo importa.

Mesmo assim, pode reconhecer que o Oshino anterior considerava aquilo importante.

\---

# 3\. Prioridades registradas

1. Encontrar **Sr. Veríssimo**.
2. Encontrar **Jonas Steal**.
3. Descobrir por que Jonas é necessário para “resolver isso tudo”.
4. Impedir que Kian permaneça livre sem oposição.
5. Investigar as cinco Âncoras.
6. Investigar o Ciclo.
7. Investigar Otávio Fritz.
8. Monitorar as próprias inscrições.
9. Não depender cegamente do Outro Lado.
10. Registrar toda informação crítica.

\---

# 4\. Protocolo de aliados

Até nova avaliação:

* não sacrificar aliados por conveniência;
* não considerar ausência de empatia justificativa suficiente para crueldade;
* preservar decisões morais tomadas antes da perda dos sentimentos;
* consultar registros anteriores quando houver dúvida sobre relações pessoais;
* não tomar decisões irreversíveis baseadas apenas em eficiência.

\---

# 5\. Protocolo de civis

Até nova avaliação:

* evitar dano desnecessário;
* não usar civis como recursos descartáveis;
* preservar vidas quando isso não comprometer diretamente a sobrevivência da equipe;
* registrar exceções e justificativas.

\---

# 6\. Protocolo paranormal

* Não aceitar nova negociação com o Outro Lado sem avaliar o preço.
* Não assumir que um preço é único ou final.
* Comparar qualquer nova alteração com o Caso Fritz.
* Registrar mudanças de memória, identidade, autonomia ou comportamento.
* Informar pelo menos um aliado caso surjam sinais de perda de controle.

\---

# 7\. Protocolo de continuidade

Caso Oshino perceba que sua identidade está se deteriorando:

1. registrar o que mudou;
2. comparar com registros anteriores;
3. entregar cópia dos registros a um aliado confiável;
4. limitar acesso a rituais ou artefatos perigosos se houver perda de autonomia;
5. investigar reversão;
6. impedir que informações críticas desapareçam com ele.

\---

# 8\. Frases de referência

> \*\*“Eu não preciso sentir que uma coisa é importante para saber que ela é.”\*\*

> \*\*“Ausência de emoção não é ausência de responsabilidade.”\*\*

> \*\*“Otávio entregou o controle para obter respostas. Eu entreguei meus sentimentos. Preciso descobrir o que o Outro Lado espera depois.”\*\*

\---

# 9\. Espaço para registros futuros

## Alterações físicas

* 

## Alterações mentais

* 

## Novas inscrições

* 

## Reações da câmera

* 

## Contato com entidades

* 

## Mudanças de memória

* 

## Decisões que contradizem registros anteriores

* 

